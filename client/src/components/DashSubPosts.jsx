...
function DashSubPosts() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        const res = await fetch(`/api/post/getuserposts?userId=${currentUser._id}`);
    ...
    if(currentUser.isAdmin || currentUser.isSub) {
      fetchPosts(); 
    }
  }, [currentUser._id]);
  ...
  async function handleShowMore() {
    ...
    try {
      const res = await fetch(
        `/api/post/getuserposts?userId=${currentUser._id}&startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
